...
export async function transferFunds(_: unknown, formData: FormData) {
  const user = await requireUser();
  const payload = transferSchema.parse(Object.fromEntries(formData.entries()));
  if (!hasSupabaseEnv) {
    revalidatePath("/dashboard");
    revalidatePath("/dashboard/transfers");
    return { ok: true, demo: true, user: user.full_name, payload };
  }
...
export async function payBill(_: unknown, formData: FormData) {
  await requireUser();
...
export async function buyAirtime(_: unknown, formData: FormData) {
  await requireUser();
...
export async function createSupportTicket(_: unknown, formData: FormData) {
  const user = await requireUser();
...
export async function updateProfileAction(formData: FormData) {
  ...
  if (!hasSupabaseEnv) {
    ...
    return { ok: true, demo: true, full_name, phone };
  }
  const supabase = createSupabaseServerClient();
